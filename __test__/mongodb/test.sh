...
$NIX_INFRA cluster cmd -d $WORK_DIR --target="worker001" "printf 'app-mongodb-pod: '; curl --max-time 2 -s 'http://127.0.0.1:11311/db?id=1&message=hello'" # > 1
$NIX_INFRA cluster cmd -d $WORK_DIR --target="worker001" "printf 'app-mongodb-pod: '; curl --max-time 2 -s 'http://127.0.0.1:11311/db?id=2&message=bye'" # > 2
$NIX_INFRA cluster cmd -d $WORK_DIR --target="worker001" "printf 'app-mongodb-pod: '; curl --max-time 2 -s 'http://127.0.0.1:11311/db?id=3&message=hello_world'" # > 3

$NIX_INFRA cluster cmd -d $WORK_DIR --target="worker001" "printf 'app-mongodb-pod: '; curl --max-time 2 -s http://127.0.0.1:11311/db/1" # > hello
$NIX_INFRA cluster cmd -d $WORK_DIR --target="worker001" "printf 'app-mongodb-pod: '; curl --max-time 2 -s http://127.0.0.1:11311/db/2" # > bye
$NIX_INFRA cluster cmd -d $WORK_DIR --target="worker001" "printf 'app-mongodb-pod: '; curl --max-time 2 -s http://127.0.0.1:11311/db/3" # > hello_world
...
$NIX_INFRA cluster cmd -d $WORK_DIR --target="worker001" "journalctl -n 60 -u podman-app-mongodb-pod"
