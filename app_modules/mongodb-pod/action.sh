...
if [ "$CMD" = "init" ]; then
  ...
  podman exec mongodb-pod $MONGO_SHELL --port 27017 --eval "rs.initiate({_id: \"rs0\", members: [{_id: 0, host: \"$NODE_1\", priority: 100},{_id: 1, host: \"$NODE_2\"},{_id: 2, host: \"$NODE_3\"}]})"
  echo "Result Code: $?"
fi

if [ "$CMD" = "status" ]; then
  podman exec mongodb-pod $MONGO_SHELL --quiet --port 27017 --eval 'rs.status()'
  echo "Result Code: $?"
fi
...
if [ "$CMD" = "dbs" ]; then
  echo "dbs..."
  if [ -z "$VERBOSE" ]; then
    podman exec mongodb-pod $MONGO_SHELL --quiet --port 27017 --eval "$(cat <<EOF
...
# if [ "$CMD" = "shell" ]; then
#   podman exec --interactive mongodb-pod $MONGO_SHELL --port 27017 --shell
# fi
