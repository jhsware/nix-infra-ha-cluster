...
MARIADB_ROOT_PASSWORD=$(printenv MARIADB_ROOT_PASSWORD || echo "")
...
if [ "$CMD" = "init" ]; then
  ...
  if [ -n "$NODE_1" ]; then
    ...
    mariadb -u$MARIADB_ROOT_USER -p"$MARIADB_ROOT_PASSWORD" -e "SHOW STATUS LIKE 'wsrep_cluster_size';"
  else
...
if [ "$CMD" = "status" ]; then
  echo "Cluster Status:"
  # mariadb -u$MARIADB_ROOT_USER -p"$MARIADB_ROOT_PASSWORD" -e "SHOW STATUS LIKE 'wsrep%';"
  mariadb -u$MARIADB_ROOT_USER -p"$MARIADB_ROOT_PASSWORD" -e "SHOW GLOBAL STATUS WHERE Variable_name IN ('wsrep_local_state_comment', 'wsrep_ready', 'wsrep_cluster_size', 'wsrep_cluster_status', 'wsrep_connected');"
  echo "Result Code: $?"
fi
